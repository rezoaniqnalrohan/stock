...
@section('subheading', $order->customer->name.' · from '.$order->warehouse->name)

@section('actions')
    @if ($order->status === 'pending')
        <form method="POST" action="/orders/{{ $order->id }}/fulfill" onsubmit="return confirm('Fulfil order and decrement stock?')">
            @csrf
...
<div class="grid lg:grid-cols-3 gap-4">
    <x-card title="Order info" class="lg:col-span-1">
        <dl class="space-y-3 text-sm">
            <div class="flex justify-between"><dt class="text-slate-500">Status</dt><dd><x-badge :color="['pending' => 'amber', 'fulfilled' => 'blue', 'shipped' => 'green'][$order->status]">{{ ucfirst($order->status) }}</x-badge></dd></div>
            <div class="flex justify-between"><dt class="text-slate-500">Customer</dt><dd class="font-medium text-slate-800">{{ $order->customer->name }}</dd></div>
                ...
                <tbody class="divide-y divide-slate-100">
                    @foreach ($order->items as $it)
                        <tr>
                            ...
                            <td class="py-2.5 px-2 text-right tabular-nums">${{ number_format($it->unit_price, 2) }}</td>
                            <td class="py-2.5 px-2 text-right tabular-nums font-semibold">${{ number_format($it->quantity * $it->unit_price, 2) }}</td>
                        </tr>
                    @endforeach
                </tbody>
            </table>
